...
public class Physics : SystemBase
{
    ...
    private bool CheckCollision(int serial)
    {
        ...
        {
            if (serial != ramdomSerial[i])
            {
                if (IsCollisionBall(currentPositionBall[serial] + currentVelocityBall[serial], currentPositionBall[ramdomSerial[i]]))
                {
                    VelocityAfterCollisionBall(currentPositionBall[serial], currentPositionBall[ramdomSerial[i]], currentVelocityBall[serial], currentVelocityBall[ramdomSerial[i]],
    ...
    public static void GetHitPositionBallToBall(float3 posStart, float3 posTarget, float3 force, out bool isHit, out float3 result)
    {
        ...
        else
        {
            ...
            y = (-a * x2 - c) / b;
            float3 result2 = new float3(x2, 0, y);
            ...
            if (posStart.x > result2.x && posStart.x > nextPos.x || posStart.x < result2.x && posStart.x < nextPos.x)
            {
                if (posStart.z > result2.z && posStart.z > nextPos.z || posStart.z < result2.z && posStart.z < nextPos.z)
                {
                    isHit = true;
                }
            }
